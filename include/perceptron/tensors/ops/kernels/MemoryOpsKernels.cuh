...
void
copy_if_kernel(dim3 blocks, dim3 threads, size_type shared_mem, cudaStream_t stream,
  ...
  details::copy_if_kernel_imlp<<<blocks, threads, shared_mem, stream>>>(*src_pinned_owner, utils::device_forward<Predicate>(predicate), *dst_pinned_owner);
}
...
void
set_if_kernel(dim3 blocks, dim3 threads, size_type shared_mem, cudaStream_t stream,
              T value, Predicate &&predicate, TensorWriteable2D<T> dst) {
  auto &&dst_pinned_owner = utils::cu_make_pinned_memory_unique(&dst);
  details::set_if_kernel_impl<<<blocks, threads, shared_mem, stream>>>(value, utils::device_forward<Predicate>(predicate), *dst_pinned_owner);
}
