...
private:
  std::shared_ptr<activations::IActivation> m_activation;
  std::unique_ptr<optimizers::IOptimizer> m_weights_optimizer;
  tensors::TensorOwner2D<float> m_weights;

  std::optional<std::unique_ptr<optimizers::IOptimizer>> m_bias_optimizer{std::nullopt};
  std::optional<tensors::TensorOwner1D<float>> m_bias{std::nullopt};

  std::optional<tensors::TensorOwner2D<float>> m_neurons{std::nullopt};
  std::optional<tensors::TensorOwner2D<float>> m_active_neurons{std::nullopt};
...
Layer::Layer(size_type input_layer, size_type n_neurons,
    ...
    : m_activation(activation),
      ...
      m_weights(tensors::constructTensorOwnerDevice2D<float>(input_layer, n_neurons, stream)),
      m_bias(std::nullopt) {
  ...
  if (config.has_value()) {
    ...
    if (auto bias_it = config_value.find("with_bias");
        bias_it == config_value.cend() || std::any_cast<bool>(bias_it->second)) {
      m_bias_optimizer = optimizers::OptimizerFactory::build(optimizer_describer);
      m_bias = tensors::constructTensorOwnerDevice1D<float>(n_neurons, stream);
      tensors::ops::generate(utils::curand_uniform_tag{-xavier_lim, xavier_lim},
...
Layer::transform_impl(tensors::TensorReadOnly2D<float, trans> inputs) {
  ...
  if (with_bias()) {
    tensors::ops::add_row(1.0f, m_bias->tensor_view().to_read_only(),
                          1.0f, neurons_view);
  }
...
Layer::update_gradients_impl(tensors::TensorReadOnly2D<float, transa> forwards,
  ...
  if (forwards_t.get_ncols() != errors.get_nrows()) {
    std::stringstream stream;
    stream << "Bad forwards and errors shapes, must be shape for (f^T @ e), but have " << forwards.shape_repr()
           << " and " << errors.shape_repr();
    throw std::invalid_argument{stream.str()};
  }
  auto weights_view = m_weights.tensor_view();
  if (weights_view.get_nrows() != forwards_t.get_nrows() ||
      weights_view.get_ncols() != errors.get_ncols()) {
    std::stringstream stream;
    stream << "Bad forwards and errors shapes, must be shape for (f^T @ e).shape == weights.shape, but have "
           << forwards.shape_repr()
           << " and " << errors.shape_repr() << ", weights shape: " << weights_view.shape_repr();
    throw std::invalid_argument{stream.str()};
  }
  ...
  if (with_bias()) {
    ...
    auto ones_vector_owner = tensors::constructTensorOwnerDevice1D<float>(errors_t.get_ncols());
    auto ones_vector_view = ones_vector_owner.tensor_view();
    tensors::ops::set(1.0f, ones_vector_view.to_2d());
    ...
    tensors::ops::gemv(1.0f, errors_t, ones_vector_view.to_read_only(),
                       0.0f, bias_grads_view);
...
Layer::compute_errors_impl(tensors::TensorReadOnly2D<float, transa> next_layer_errors,
  ...
  tensors::ops::gemm(1.0f, next_layer_errors, next_layer_weights_t,
                     0.0f, backward_errors_view);
